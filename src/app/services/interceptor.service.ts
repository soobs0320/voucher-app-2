...
export class InterceptorService implements HttpInterceptor {
  ...
  private async handle401(req: HttpRequest<any>, next: HttpHandler) {
    const refreshTokenFromStorage = await this.storageService.get('refreshToken');
    ...
    const accessToken = await this.erpService.getAccessToken(refreshToken.data).catch((err) => {
